...
def paint_pcl(in_path, out_path, args):
    ...
    pcl = np.load(in_path+'_vert.npy')#读入场景点云数据（n, 6）
     
    

    colors = pcl[:,3:] 
    colors = colors/256 #(40000,3)
    ...
    #bbox related
    bbox=np.load(in_path+'_bbox.npy') #读入GT建议框数据(M, 7)
    print(bbox.shape)
    bbox_corners = np.zeros([bbox.shape[0], 8, 3]) #cornor (M,8,3)
    for k in range(len(bbox)):
        bbox_corners[k] = box_center_to_corner(bbox[k])

    bbox_corners[...,[0, 1, 2]] = bbox_corners[..., [0, 2, 1]]  # box_center_to_corner中对坐标系进行了变换，这里要进行rotate所以先变回来和pcl一致
    bbox_corners[..., 2] *= -1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", default="2500x2500", type=str)  #渲染分辨率 rendering resolution
    ...
    k = 0 #the index of scenes
    scene_num = 'scene'+('000'+str(k)+'_00')[-7:]
